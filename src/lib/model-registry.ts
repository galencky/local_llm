...
const DEFAULT_LADDER: ModelSpec[] = [
  ...
  { id: "gemini-2.5-flash", label: "2.5 Flash", tier: "flagship", dailyLimit: 20 },
  ...
  { id: "gemini-2.5-flash-lite", label: "2.5 Flash Lite", tier: "lite", dailyLimit: 20 },
];
...
export function markUnavailable(
  ...
  const until = daily
    ? nextPacificMidnight()
    : Date.now() + (opts.retryAfterMs ?? 60_000);
...
export function availability(): ModelAvailability[] {
